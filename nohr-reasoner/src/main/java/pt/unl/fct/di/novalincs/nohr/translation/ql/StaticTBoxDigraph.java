...
/**
 * Implementation of {@link TBoxDigraph}. Note that <i>&Psi;(T)</i> and <i>&Omega;(T)</i> are computed only when the corresponding methods are called
 * for the first time. This {@link TBoxDigraph} is static in the sense that can represent only the TBox digraph of an ontology in a given moment; if
 * the ontology changes other {@link StaticTBoxDigraph} must be constructed.
 *
 * @author Nuno Costa
 */
public class StaticTBoxDigraph implements TBoxDigraph {
	...
	/**
	 * The mapping between concept vertices and the set of their predecessor {@link GraphClosure closure} in the concepts subgraph (i.e. the subgraph
	 * containing only the vertices that represent concepts) of this {@link TBoxDigraph}.
	 */
	private final Map<OWLClassExpression, Set<OWLClassExpression>> conceptsPredecessors;

	/**
	 * The set of irreflexive roles <i>&Psi;(T)</i> of the the TBox, <i>T</i>, of which this {@link TBoxDigraph} is <i>digraph</i> (see <b>Definition
	 * 7.</b> of {@link <a href=" http://centria.di.fct.unl.pt/~mknorr/ISWC15/resources/ISWC15WithProofs.pdf">Next Step for NoHR: OWL 2 QL</a>}). May
	 * be empty if <i>&Pse;(T)</i> wasn't yet been computed.
	 */
	...
	/**
	 * The mapping between roles vertices and the set of their predecessor {@link GraphClosure closure} in the roles subgraph (i.e. the subgraph
	 * containing only the vertices that represent roles) of this {@link TBoxDigraph}.
	 */
	 ...
	 * QL</a>}). May be empty, if <i>&Omega;(T)</i> wasn't yet been computed.
	 */
	 ...
	 *            a set <i>S<sub>2</sub>.
	 * @param <E>
	 *            the type of the elements of {@code s1} and {@code s2}.
	...
	/**
	 * Obtains the the sets of the atomic roles that occurs in an existential of one given basic concepts set and whose inverse occurs in an
	 * existential of another basic concepts set.
	 *
	 * @param s1
	 *            a set of basic concepts <i>S<sub>1</sub></i>.
	 * @param s2
	 *            a set of basic concepts <i>S<sub>2</sub></i>.
	 * @return <i> {P | &exist;P &in; S<sub>1</sub> and &exist;P<sup>-</sup> &in; S<sub>2</sub> } &cup; <i> {P | &exist;P<sup>-</sup> &in; S
	...
	/**
	 * Obtains the the sets of the atomic roles of one given basic roles set whose inverse belongs to another basic roles set.
	 *
	 * @param s1
	 *            a set of basic roles <i>S<sub>1</sub></i>.
	 * @param s2
	 *            a set of basic roles <i>S<sub>2</sub></i>.
	 * @return <i> {P | P &in; S<sub>1</sub> and P<sup>-</sup> &in; S<sub>2</sub> } &cup; <i> {P | P<sup>-</sup> &in; S <sub>1</sub> and P &in; S
	 *         <sub>2</sub> } </i>
	 */
