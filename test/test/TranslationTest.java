...
public class TranslationTest {
	...
	private Translation tr(OWLAxiom axiom, HashSet<Rule> ... ruleSets) throws OWLOntologyCreationException {
	...
	private HashSet<Rule> subsumptionRules(OWLObjectProperty P1, OWLObjectProperty P2) {
		HashSet<Rule> result = new HashSet<Rule>();
	...
	private HashSet<Rule> disjunctionRules(OWLObjectProperty P1, OWLObjectProperty P2) {
		HashSet<Rule> result = new HashSet<Rule>();
	...
	public void core() throws OWLOntologyCreationException,
		...
		Translation[] translations = {
				...
				tr(df.getOWLSubClassOfAxiom(A[3], some(P[1])),
						...
						rule("dom(d%s)(X):-d%s(X),tnot(n%s(X,_)).", P[1], A[3], P[1]),
						rule("n%s(X):-n%s(X,_).", A[3], P[1])),
				
				tr(df.getOWLSubClassOfAxiom(A[4], some(inv(P[2]))),
						rule("ran(a%s)(X):-a%s(X).", P[2], A[4]),
						rule("ran(d%s)(X):-d%s(X),tnot(n%s(_,X)).", P[2], A[4], P[2]),
						rule("n%s(X):-n%s(_,X).", A[4], P[2])),
				
				tr(df.getOWLSubClassOfAxiom(some(P[3]), A[5]),
				        rule("a%s(X):-dom(a%s)(X).", A[5], P[3]),
				        rule("d%s(X):-dom(d%s)(X),tnot(n%s(X)).", A[5], P[3], A[5]),
				        rule("n%s(X,_):-n%s(X).", P[3], A[5])),
				        
				tr(df.getOWLSubClassOfAxiom(some(inv(P[4])), A[6]),
						rule("a%s(X):-ran(a%s)(X).", A[6], P[4]),
						rule("d%s(X):-ran(d%s)(X),tnot(n%s(X)).", A[6], P[4], A[6]),
						rule("n%s(_,X):-n%s(X).", P[4], A[6])),
				...
				tr(df.getOWLSubObjectPropertyOfAxiom(P[7], inv(P[8])),
						...
						rule("d%s(Y,X):-d%s(X,Y),tnot(n%s(Y,X)).", P[8], P[7], P[8]),
						rule("n%s(X,Y):-n%s(Y,X).", P[7], P[8])),
						
				tr(df.getOWLSubObjectPropertyOfAxiom(inv(P[9]), P[10]),
						rule("a%s(X,Y):-a%s(Y,X).", P[10], P[9]),
						rule("d%s(X,Y):-d%s(Y,X),tnot(n%s(X,Y)).", P[10], P[9], P[10]),
						rule("n%s(Y,X):-n%s(X,Y).", P[9], P[10])),
						
				tr(df.getOWLSubClassOfAxiom(A[7], df.getOWLObjectComplementOf(A[8])),
	...
	private OWLObjectSomeValuesFrom some(OWLObjectPropertyExpression rule, OWLClass filler) {
	...
	public void extended() throws OWLOntologyCreationException,
		...
		OWLIndividual a = kb.getIndividual();
		OWLIndividual b = kb.getIndividual();
		OWLSubClassOfAxiom someSub = df.getOWLSubClassOfAxiom(A[4], df.getOWLObjectSomeValuesFrom(P[0], A[5]));
		...
		Translation[] translations = {	
			   ...
			   tr(df.getOWLSubClassOfAxiom(A[0], and(A[1], A[2], A[3])),	
				...
				tr(someSub, 
						...
						rule("d%s(X,Y):-d%s(X,Y),tnot(n%s(X,Y)).", P[0], pSomeSub, P[0]),
						rule("n%s(X,Y):-n%s(X,Y).", pSomeSub, P[0]),
						rule("a%s(X):-ran(a%s)(X).", A[5], pSomeSub), 
						rule("d%s(X):-ran(d%s)(X),tnot(n%s(X)).", A[5], pSomeSub, A[5]), 
						rule("n%s(_,X):-n%s(X).", pSomeSub, A[5]),
						rule("dom(a%s)(X):-a%s(X).", pSomeSub, A[4]),
						rule("dom(d%s)(X):-d%s(X),tnot(n%s(X,_)).", pSomeSub, A[4], pSomeSub),
				...
				tr(df.getOWLInverseObjectPropertiesAxiom(P[1], P[2]),
						...
						rule("d%s(Y,X):-d%s(X,Y),tnot(n%s(X,Y)).", P[2], P[1], P[2]),
						rule("n%s(X,Y):-n%s(Y,X).", P[1], P[2]),
						rule("a%s(Y,X):-a%s(X,Y).", P[1], P[2]),
						rule("d%s(Y,X):-d%s(X,Y),tnot(n%s(Y,X)).", P[1], P[2], P[1]),
						rule("n%s(X,Y):-n%s(Y,X).", P[2], P[1])
				),
				...
				tr(df.getOWLObjectPropertyDomainAxiom(P[6], A[10]),
						...
						rule("d%s(X):-dom(d%s)(X),tnot(d%s(X)).", A[10], P[6], A[10]),
						rule("n%s(X,_):-n%s(X).", P[6], A[10])),
						
				tr(df.getOWLObjectPropertyRangeAxiom(P[7], A[11]),
						rule("a%s(X):-ran(a%s)(X).", A[11], P[7]),
						rule("d%s(X):-ran(d%s)(X),tnot(d%s(X)).", A[11], P[7], A[11]),
						rule("n%s(_,X):-n%s(X).", P[7], A[11])),
...
//						rule("n%s(X,_):-a%s(_,X).", P[13], P[13])),
	...
	public void ignored() throws OWLOntologyCreationException,
		...
		Translation [] translations =  {
				...
				tr(df.getOWLObjectPropertyAssertionAxiom(df.getOWLTopObjectProperty(), a, b)),
				tr(df.getOWLObjectPropertyAssertionAxiom(df.getOWLBottomObjectProperty(), a, b)),
				tr(df.getOWLSubClassOfAxiom(A[0], df.getOWLThing())),
				tr(df.getOWLSubClassOfAxiom(df.getOWLNothing(), A[1])),
				tr(df.getOWLSubClassOfAxiom(A[2], df.getOWLObjectComplementOf(df.getOWLNothing()))),
				tr(df.getOWLSubClassOfAxiom(df.getOWLNothing(), df.getOWLObjectComplementOf(A[3]))),
				tr(df.getOWLSubObjectPropertyOfAxiom(P[0], df.getOWLTopObjectProperty())),
				tr(df.getOWLSubObjectPropertyOfAxiom(df.getOWLBottomObjectProperty(), P[1])),
				//not in profile
				//tr(df.getOWLDisjointObjectPropertiesAxiom(P[1], df.getOWLBottomObjectProperty()))
		};
	...
	public void special() throws OWLOntologyCreationException,
		...
		Translation [] translations =  {
				tr(df.getOWLSubClassOfAxiom(A[0], df.getOWLNothing()),
						disjunctionRules(A[0], A[0])),
						
				tr(df.getOWLSubClassOfAxiom(A[1], df.getOWLObjectComplementOf(df.getOWLThing())),
						disjunctionRules(A[1], A[1])),
						
				tr(df.getOWLSubClassOfAxiom(df.getOWLThing(), df.getOWLObjectComplementOf(A[2])),
						disjunctionRules(A[2], A[2])),
						
				tr(df.getOWLSubObjectPropertyOfAxiom(P[0], df.getOWLBottomObjectProperty()),
						disjunctionRules(P[0], P[0])),		
				
//not in profile
//				tr(df.getOWLDisjointObjectPropertiesAxiom(P[1], df.getOWLTopObjectProperty()),
